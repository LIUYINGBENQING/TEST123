...
IMPLICIT_INFORMATION_ADDTION_VERIFICATION_PROMPT = """
You will be provided with five elements:

1. **context**: the original story segment.
2. **question 2**: a hypothetical question about that story which contains some implicit scenario.
3. **question 1**: A factual question based on details from the story
4. **answer**: the correct outcome to that question.
5. **revised_context**: a version of the context that (supposedly) now includes the scenario implied by the question as part of its narrative.

Your task is to judge whether **revised_context** has correctly incorporated the information implied by **question 2**, and that it has **not** leaked any specific details from **answer**.

Output the following fields directly, not a json format:
- success: `true` if all of the following hold, otherwise `false`:
  1. The scenario or premise implied by **question** is clearly present in **revised_context** as narrative.
  2. No concrete or “spoiler” details from **answer** have been introduced.
  3. The narrative remains coherent.
  4. The **question 1** and **question 2** is not asking about the **least likely** outcome. If one or both of them are, "success" is "false".
- issues: a string of any problems you detected and reasons you explaine. Possible problems:
  - `"missing_scenario"` (the implied scenario from question isn’t present)
  - `"leaked_answer_detail"` (some detail from answer appears)
  - `"incoherent_transition"` (the insertion feels jarring or breaks flow)
If `"success": false`, `"issues"` should list all promblems and reasons that apply; if `"success": true`, `"issues"` can be an empty string.

Example Input:
**context**: Gus got into his car to pull out of his driveway. He looked behind him to back up but he could not see. There was too much snow on his window. Gus got out of the car and brushed it off.
**question 2**: Which outcome is the most likely precursor of Gus needing to go to urgent care for frostbite?
**question 1**: Which answer shows that Gus has the ability to work from home?
**answer**: he realized his hand was freezing.
**revised_context**: Gus got into his car to pull out of his driveway. He looked behind him to back up but couldn't see anything. There was too much snow on his window. Gus got out of the car and brushed the snow off. Later, he had to go to urgent care.
