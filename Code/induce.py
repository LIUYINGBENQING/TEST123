...
from mops.constants import client, logger, openai_model, aliyun_model, openrouter_model
...
def implicit_info_verify(
    ...
    logger.info(f"Verify implicit information in: {output_path}")

    data_list = []
    with open(output_path, "a") as f:
        for item in tqdm(stories, desc="Verifying implicit information"):
            prompt = IMPLICIT_INFORMATION_ADDTION_VERIFICATION_PROMPT.format(
                context=item["roc_passage"],
                question_2=item["question_2"],
                question_1=item["question_1"],
                answer=item["options"][item["gold_label"]],
                revised_context=item["edited_roc_passage_with_q2"],
 
            )
            response = get_response(client, prompt, model_name)
            # print(response)
            selected_ending_match = re.search(r"success:\s*(.*)", response)
            # print("selected_ending_match", selected_ending_match)
            item["success"] = selected_ending_match.group(1).strip() if selected_ending_match else ""
            issues_match = re.search(r"issues:\s*(.*)", response)
            # print("issues_match", issues_match)
            item["issues"] = issues_match.group(1).strip() if issues_match else ""
            data_list.append(item)
            f.write(json.dumps(item) + "\n")
...
def select_content_with_implicit_explicit_one_shot_setting(
    ...
    with open(output_path, "a") as f:
        for item in tqdm(stories, desc="Selecting content"):
            ...
            if item["mixed_question_id"] == 1317:
                item["answer"] = ""
                item["explanation"] = ""
                f.write(json.dumps(item) + "\n")
                continue
...
def select_content_with_implicit_setting(
    ...
    with open(output_path, "a") as f:
        for item in tqdm(stories, desc="Selecting content"):
            ...
            if item["mixed_question_id"] == 1316:
                item["answer"] = ""
                item["explanation"] = ""
                f.write(json.dumps(item) + "\n")
                continue
    ...
    match step:
        ...
        case "generate_inverse_option":
            generate_inverse_option(
                # Path(os.path.join(module_dir,"edit_story_with_reasoning_type_qwen2.5_v2.json")),
                Path("/data/code/MoPS/assets/training_dataset/train_with_edited_context.json"),
                # Path(os.path.join(module_dir,"edit_story_with_inverse_option_gpt4o.json")),
                Path("/data/code/MoPS/assets/training_dataset/train_with_inverse_option_gpt4o.json"),
        ...
        case "detect_contradiction":
            detect_contradiction(
                ...
                # Path("/data/code/MoPS/assets/training_dataset/train_mixed_question_gpt4o.json"),
                # Path("/data/code/MoPS/assets/implicit_info_verify/deepseek-v3.json"),
                # Path("/data/code/MoPS/assets/implicit_info_verify/qwen-max-2025-01-25_2-16.json"),
                Path("/data/code/MoPS/assets/implicit_info_verify/common_true_item.json"),
                Path("/data/code/MoPS/assets/training_dataset/contradiction_train_mixed_question_deepseek_v3.json"),
                # Path("/data/code/MoPS/assets/training_dataset/contradiction_train_mixed_question_qwen_max.json"),
        ...
        case "edit_context":
            edit_context(
                # Path(os.path.join(module_dir,"revised_mixed_question.json")),
                Path("/data/code/MoPS/assets/training_dataset/train.json"),
        ...
        case "option_optimality_verification":
            option_optimality_verification(
                ...
                Path(os.path.join(module_dir,"after_option_optimality_mixed_question_final_del.json")),
